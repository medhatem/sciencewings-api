"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.LoggerStorage = void 0;
const LocalStorage_1 = require("@utils/LocalStorage");
const uuid_1 = require("uuid");
/**
 * method decorator that sets an id into the local storage
 * the id is meant for tracing logs
 *
 * it is typicaly used on routes to create a unique id
 * and stace all the logs from that route
 * which improves debugging
 *
 *
 */
function LoggerStorage() {
    return (target, propertyKey, descriptor) => {
        const originalFunction = descriptor.value;
        descriptor.value = function (...args) {
            return __awaiter(this, void 0, void 0, function* () {
                LocalStorage_1.LocalStorage.getInstance().enterWith({ id: (0, uuid_1.v4)() }); //add a new uuid to the storage
                return originalFunction.apply(this, args);
            });
        };
        return descriptor;
    };
}
exports.LoggerStorage = LoggerStorage;
//# sourceMappingURL=loggerStorage.js.map