...
suite(__filename.substring(__filename.indexOf('/server-test') + '/server-test/'.length), (): void => {
  ...
  suite('invite User By Email', async () => {
    ...
    test('Should fail on retriving organization', async () => {
      ...
      try {
        await container.get(MemberService).inviteUserByEmail({ email, organizationId: orgId, roles: ['regular'] });
    ...
    test('Should succeed on invite user', async () => {
      ...
      const result = await memberService.inviteUserByEmail({ email, organizationId: orgId, roles: ['regular'] });
